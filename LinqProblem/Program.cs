...
// 전체 나이가 평균이상인 사람이 2022년 8월 짝수의 날짜에 섭취한 음식을 중복값 없이 앞에서 4번째부터 출력해주세요.

var eatFoodEvenDay = people
    .Where(x => x.Age > people.Average(x => x.Age))
    .SelectMany(x => x.EatenList)
    .Where(x => Convert.ToInt32(x.EatenDate.Day) % 2 == 0)
    .SelectMany(x => x.Foods
        .Select(x => x.Name))
    .Distinct()
    .Skip(3).ToList();

foreach (var item in eatFoodEvenDay)
{
...
// 나이가 제일 어린 사람이 섭취한 칼로리 음식중 제일 많은 양의 단백질 비율을 가진 음식의 단백질율을 구하세요.
// 단백질율 : 단백질양/칼로리양* 100

var getProteinRatio = people
    .Where(x => x.Age == people.Min(x => x.Age))
    .SelectMany(Person => Person.EatenList
    .SelectMany(EatenList => EatenList.Foods))
    .Max(x => x.Nutrition.Protein.Value / x.Nutrition.Calories.Value * 100);

Console.WriteLine(getProteinRatio);

Console.WriteLine("-----------------------------------------");

// 개인별 가장 많이 먹은 음식을 찾고 아래와 같이 나열해주세요!
// 이름 : 음식이름

var peopleEatMostFood = people.Select(x => new
{
    x.Name,
    mostFood = x.EatenList
        .SelectMany(x => x.Foods)
        .GroupBy(x => x.Name)
...
foreach (var item in peopleEatMostFood)
{
    Console.WriteLine($"{item.Name} : {item.mostFood}");
}

Console.WriteLine("-----------------------------------------");

//모든 사람들이 먹은 음식들 중 개인별 먹지 않은 음식이 어떤것이 있는지 아래처럼 나열해주세요
// 이름 : {이름} / 안먹은 음식들 : 안먹은음식1, 안먹은음식2 ...

var peopleNotEatFood = people
    .Select(x => new
    {
        x.Name,
        toTalEatFood = people
            .SelectMany(x => x.EatenList
                .SelectMany(x => x.Foods
                    .Select(x => x.Name)))
                .Distinct(),
        eatAloneFood = x.EatenList
            .SelectMany(x => x.Foods
                .Select(x => x.Name))
            .Distinct()
    }).Select(x => new { x.Name, exceptsFood = x.toTalEatFood.Except(x.eatAloneFood).Aggregate((a, b) => a + "," + b) });

foreach (var item in peopleNotEatFood)
{
    Console.WriteLine($"이름 : {item.Name} / 안먹은 음식들 : {item.exceptsFood}");
}
